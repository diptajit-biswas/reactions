    ...
    public static function createPaymentIntent(object $body, int $amount): PaymentIntent
    {
        try {
            ...
            if (!empty($body->paymentMethodId)) {
                $intent = new \Stripe\StripeClient(
                  'sk_test_51HFjfaCdK04u03XMSiR5JMUaKRoyF3QeIbzQYIYjHbZ5kgpn7H3wDVVivWrztQiugQewMc5tuMHogIVHIZUWSjVj00Ez9hiEDz'
                );
                $intentResponce = $intent->paymentIntents->create([
                    'amount' => $amount,
                    'currency' => STRIPE_CURRENCY,
                    'payment_method' => $body->paymentMethodId,
                    'confirmation_method' => 'manual',
                    'confirm' => true,
                    'receipt_email' => $body->recipientEmail,
